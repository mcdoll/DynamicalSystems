...
theorem blubb (hv : Differentiable ℝ v) {s : Set E} (hs : IsInvariantSet (Φ ·) s) (c : ℝ)
    (hsv : ∀ x ∈ s, v x = c) {x : E} (hx : x ∈ s) : fderiv ℝ v x (f x) = 0 := calc
...
theorem blubb' {c : ℝ} (h_tendsto : Filter.Tendsto (fun t ↦ v (Φ t y)) Filter.atTop (𝓝 c)) {x : E}
  ...
  have : MapClusterPt (v x) Filter.atTop fun t ↦ v (Φ t y) := by
    ...
    refine Filter.tendsto_atTop_atTop_of_monotone (fun ⦃a b⦄ a_1 ↦ a_1) ?_
    intro t
    use Real.toNNReal t
    simp
...
theorem foo'' (hv : Differentiable ℝ v) (h_neg : ∀ x, fderiv ℝ v x (f x) ≤ 0) :
  ...
  let c : ℝ := 0
  apply blubb hv (Φ := Φ) (s := limitSet (Φ ·) y) _ c _ hx
  · apply isInvariantSet_limitSet (s₁ := Set.univ) (s₂ := Set.univ) (by sorry) (by sorry) (by sorry)
    sorry
  -- Then v y = c₀
  intro x' hx'
  apply blubb' Φ _ _ hx'
