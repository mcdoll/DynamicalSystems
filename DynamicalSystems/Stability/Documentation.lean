...
Asymptotic stability requires that an equilibrium point is Lyapunov stable and all trajectories
converge to the equilibrium point. In Lean, we are not defining a predicate for this, so to state
that a point is asymptotic stable, one simply writes that it is stable and all trajectories
converge. We note that in the case of the filter {lean}`nhdsSet s`, we can check convergence
similarly to the convergence of points
```lean
